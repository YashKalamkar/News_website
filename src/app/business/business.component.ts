...
export class BusinessComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._services.businessNews().subscribe((result) => {
      ...
      if (result && result.items) {
        this.businessNewsDisplay = result.items.map((item: any) => {
          if (item.timestamp) {
            item.formattedTime = this.convertTimestampToTime(Number(item.timestamp));
          }
          return item;
        });
        this.displayedNews = this.businessNewsDisplay.slice(0, this.newsToShow);
      }
    }, (error) => {
  ...
  loadMore(): void {
    ...
    const nextBatch = this.businessNewsDisplay.slice(currentLength, currentLength + this.newsToShow);
